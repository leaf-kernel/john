...
void __json_add_child(json_token *parent, json_token *child)
{
    ...
    json_token *new_child = parent->data.children.items[parent->data.children.count++];
    *new_child = *child;
}
...
void john_parse(john_json *john, const char *data)
{
    ...
    int name_index = 0;
    json_token *current_token = __JOHN_NULL__;
    json_token *parent_token = __JOHN_NULL__;
    __json_val_init(&john->root);
    current_token = &john->root;
